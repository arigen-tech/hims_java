...
public class BloodBankController {
    ...
    public ApiResponse<?> getBloodStock(
        ...
        log.info(
                "Request Params -> bloodGroupId: {}, componentId: {}, inventoryStatus: {}, " +
                        "expiryFilter: {}, collectionType: {}, viewType: {}, page: {}, size: {}",
                bloodGroupId, componentId, inventoryStatus,
                expiryFilter, collectionType, viewType, page, size
        );
        ...
        return bloodBankService.getBloodStock(req, pageable);
    }
    ...
    public ApiResponse<Page<BloodTrackingResponse>> getAllPendingBloodRequest(
        ...
        return bloodBankService.getAllPendingBloodRequest(
                page, size, patientName, wardId);
    }
    ...
    @PostMapping("/allocateBloodUnits")
    public ResponseEntity<ApiResponse<String>> allocateBloodUnits(
            @Valid @RequestBody BloodRequestAllocationRequest request) {

        log.info("Received blood unit allocation request for {} request details", request.getDetails().size());
        ApiResponse<String> response = bloodBankService.allocateBloodUnits(request);
        return ResponseEntity.ok(response);
    }

    @GetMapping("/getAllocatedBloodRequestList")
    public ApiResponse<Page<BloodAllocatedResponse>> getAllocatedBloodRequestList(
        ...
        return bloodBankService.getAllocatedBloodRequestList(
                page, size, patientName, wardId);
    }

    @PostMapping("/saveCrossmatch")
    public ResponseEntity<ApiResponse<String>> saveCrossmatch(@RequestBody BloodCrossmatchRequest request) {
        ApiResponse<String> response = bloodBankService.saveCrossmatch(request);
        return ResponseEntity.ok(response);
    }


    @GetMapping("/getPendingBloodIssue")
    public ResponseEntity<ApiResponse<Page<BloodIssueResponse>>> getPendingBloodIssue(
            @RequestParam(defaultValue = "0") int page,
            @RequestParam(defaultValue = "5") int size,
        ...
        ApiResponse<Page<BloodIssueResponse>> response =
                bloodBankService.getPendingBloodIssue(
                        page, size, requestNo, patientName, wardId);
