...
public class LabRegistrationServicesImpl implements LabRegistrationServices {
    ...
    /**
     * Build full user name from first, middle, and last names
     */
...
//}
        ...
        try {
            ...
            for (Map.Entry<LocalDate, List<LabRadioInvestigationRequest>> entry : grouped.entrySet()) {
                ...
                DgOrderHd savedHd = labHdRepository.save(buildOrderHd(
                        savedPatient, savedVisit, currentUser, date, today, now, userFullName
                ));
    ...
    private DgOrderHd buildOrderHd(Patient patient,
        ...
        try {
            ...
            hd.setSource("lab source");
            hd.setDiscountId(1);
            hd.setCreatedBy(userFullName);
            hd.setLastChgBy(userFullName);
...
//    public ApiResponse<AppsetupResponse> savesample(SampleCollectionRequest sampleReq) {
        ...
        try {
            ...
            List<DgOrderDt> orderDetails =
                    labDtRepository.findByOrderhdIdId(sampleReq.getOrderHdId());
            ...
            // 7️⃣ UPDATE ORDER HEADER STATUS (UNCHANGED)
            boolean allCompleted = true;
            boolean anyCompleted = false;
            ...
            labHdRepository.findById(sampleReq.getOrderHdId())
                    .ifPresent(hd -> {
    ...
    public String generateSampleId(
            String modalityCode,      // e.g. "BIO"
            int dailySequence,        // e.g. 1 → 0001
            String containerSuffix      // e.g. 'A', 'B'
    ) {
